  				...
  				<div class="row align-items-start" >
  					<div class="col overflow-y-auto" style="width:400px; ">
							<div class="input-group mb-3" >
							 ...
							 $Consulta=mysql_query("SELECT * FROM `usuarios`  WHERE `status`=1  AND `id` != ".$_SESSION["id"]."  ORDER BY `app`  ASC");
              		...
              		<div class="col overflow-y-auto" style="width:400px; ">
                        ...
                        <div class="list-group overflow-y-auto" style="height:350px;" id="list_ud2">';
						 $Consulta2=mysql_query("SELECT * FROM `usuarios`  WHERE `status`=0   ORDER BY `app`  ASC");
...
';
	break;
	case 2:
	   $Consulta3=mysql_query("UPDATE `usuarios` SET `status` = '0' WHERE `id` = ".$_POST['sid'].";");
		if($Consulta3){
			echo "success";
			}
	break;
	case 3:
		 $Consulta4=mysql_query("UPDATE `usuarios` SET `status` = '1' WHERE `id` = ".$_POST['sid'].";");
		if($Consulta4){
			echo "success";
			}
	...
	case 4:
		...
		if(isset($_POST['src_txt'])){
		 ...
		 $Consulta2=mysql_query("SELECT * FROM `usuarios`  WHERE `status`=1 ".$src_txt." AND `id` != ".$_SESSION["id"]."  ORDER BY `app`  ASC");
		 while($result=mysql_fetch_array($Consulta2)){
	...
	case 5:
		...
		if(isset($_POST['src_txt'])){
		 ...
		 $Consulta2=mysql_query("SELECT * FROM `usuarios`  WHERE `status`=0 ".$src_txt."  AND `id` != ".$_SESSION["id"]."  ORDER BY `app`  ASC");
		 while($result=mysql_fetch_array($Consulta2)){
		 echo '<div class="btn-group me-2" role="group" aria-label="Second group">
		 <button type="button"  id="btn_usr_'.$result['id'].'" class=" btn-outline-secondary list-group-item list-group-item-action usr_list"   ">	 
		'.$result['app'].' '.$result['apm'].' '.$result['nombre'].'
...
<!--
<section id="win_new_user" class="win_pop2" style="width:900px; height:650px;" >
<h1 align="center" style="width:900px;" >Eliminar Usuario</h1>
<br><hr>
        ...
        <div class="input-group mb-3">
         ...
         <button class="btn btn-outline-secondary" type="button" onClick="srch_du()">Buscar</button>
        </div>

		<div class="container text-center">
  				<div class="row align-items-start" >
  					<div class="col overflow-y-auto" style="width:400px; border:1px solid #F00;">
							<div class="input-group mb-3" >
									<span class="input-group-text" id="basic-addon1" style="width:400px;">Usuarios Activos</span>
                            </div>
     						<div class="list-group overflow-y-auto" style="height:350px;" id="list_ud">
     								<div class="btn-group me-2" role="group" aria-label="Second group">
	 									<button type="button"  id="btn_usr_id" class="btn-outline-secondary list-group-item list-group-item-action usr_list"></button>
                                        <button type="button" class="btn btn-outline-secondary actv_btn"  onClick="sel_usr_del(id,2)">Eliminar</button>
                                    </div>
                            </div>
   				   </div>

       
              		<div class="col overflow-y-auto" style="width:400px; border:1px solid #00F;">
                        <div class="input-group mb-3" >
                                <span class="input-group-text" id="basic-addon1" style="width:400px;">Usuarios Eliminados</span>
                        </div>
                        <div class="list-group overflow-y-auto" style="height:350px;" id="list_ud2">
                                <div class="btn-group me-2" role="group" aria-label="Second group">
                                    <button type="button"  id="btn_usr_id" class="btn-outline-secondary list-group-item list-group-item-action usr_list"></button>
                                    <button type="button" class="btn btn-outline-secondary desc_btn"  onClick="sel_usr_del(id,3)">Restaurar</button>
                                </div>
                        </div>
               </div>
    			</div>
